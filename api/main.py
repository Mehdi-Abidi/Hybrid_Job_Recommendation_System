...
def _job_summary(row: pd.Series) -> JobSummary:
    job_id_val = row.get("job_id", row.name)
    return JobSummary(
        job_id=int(job_id_val), title=str(row.get("title", "")),
...
def recommend(user_id: int, k: int = 10, model: str = "hybrid"):
    ...
    results = []
    job_df = s.data.jobs.set_index("job_id")
    for j, sc in recs:
        row = job_df.loc[j]
        # Handle dataframe series/dataframe type if duplicate ids exist
        if isinstance(row, pd.DataFrame):
            row = row.iloc[0]
        results.append(Recommendation(job_id=j, score=sc, job=_job_summary(row)))
